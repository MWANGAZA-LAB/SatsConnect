...
router.post('/mpesa/payout', [
  ...
  body('amount')
    .isFloat({ min: 1, max: 150000 })
    .withMessage('Amount must be between 1 and 150,000 KES'),
  body('lightningInvoice')
    .isString()
    .notEmpty()
    .withMessage('Lightning invoice is required'),
  body('accountReference')
    .optional()
    .isString()
    .isLength({ min: 1, max: 20 })
...
router.get('/airtime/providers', authenticateToken, (req: express.Request, res: express.Response) => {
  res.json({
    success: true,
    data: {
      providers: [
