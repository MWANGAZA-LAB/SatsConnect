...
async fn test_lightning_engine_integration() -> Result<()> {
    ...
    let (node_id, address) = engine.create_wallet_from_mnemonic(mnemonic, "test-wallet").await?;
    
    assert!(!node_id.is_empty());
    assert!(!address.is_empty());
    println!("✅ Wallet created - Node ID: {}, Address: {}", node_id, address);
    
    // Test balance retrieval
    let (onchain_balance, lightning_balance) = engine.get_balance().await?;
    println!("✅ Balance retrieved - On-chain: {} sats, Lightning: {} sats", onchain_balance, lightning_balance);
    ...
    let (airtime_invoice, airtime_hash, status) = engine.buy_airtime(500, "254700000000", Some("Safaricom")).await?;
...
async fn test_wallet_handler_integration() -> Result<()> {
    ...
    // Test wallet creation
    let (node_id, address) = wallet_handler.create_wallet("test-wallet".to_string(), None).await?;
    assert!(!node_id.is_empty());
    assert!(!address.is_empty());
    println!("✅ Wallet handler created wallet - Node ID: {}, Address: {}", node_id, address);
    
    // Test balance retrieval
    let (onchain_balance, lightning_balance) = wallet_handler.get_balance().await?;
    println!("✅ Wallet handler balance - On-chain: {} sats, Lightning: {} sats", onchain_balance, lightning_balance);
    
    // Test invoice generation
    let (invoice, payment_hash) = wallet_handler.generate_invoice(1000, "Test invoice".to_string()).await?;
    assert!(!invoice.is_empty());
    assert!(!payment_hash.is_empty());
    println!("✅ Wallet handler invoice generated - Payment Hash: {}", payment_hash);
    
    // Test airtime purchase
    let (airtime_invoice, airtime_hash, status) = wallet_handler.buy_airtime(500, "254700000000".to_string(), Some("Safaricom".to_string())).await?;
    assert!(!airtime_invoice.is_empty());
    assert!(!airtime_hash.is_empty());
    assert_eq!(status, "PENDING");
...
async fn test_mnemonic_validation() -> Result<()> {
    ...
    let (node_id, address) = wallet_handler.create_wallet("valid-wallet".to_string(), Some(valid_mnemonic.to_string())).await?;
    assert!(!node_id.is_empty());
    assert!(!address.is_empty());
    ...
    let result = wallet_handler.create_wallet("invalid-wallet".to_string(), Some(invalid_mnemonic.to_string())).await;
