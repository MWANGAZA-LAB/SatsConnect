use anyhow::Result;
use bitcoin::{Address, BlockHash, Network, Transaction, Txid};
...
impl BitcoinClient {
    ...
    pub fn new(network: Network, rpc_url: String, rpc_user: String, rpc_password: String) -> Self {
        Self {
    ...
    pub async fn initialize(&self) -> Result<()> {
        info!(
            "Initializing Bitcoin Core client for network: {:?}",
            self.network
        );
    ...
    pub async fn estimate_fee(&self, blocks: u16) -> Result<f64> {
        ...
        Ok(client
            .estimate_smart_fee(blocks, None)?
            .fee_rate
            .unwrap_or_default()
            .to_btc())
    }
