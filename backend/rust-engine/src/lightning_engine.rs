...
impl LightningEngine {
    ...
    pub async fn initialize(&self) -> Result<()> {
        info!("Initializing Lightning Engine for network: {:?}", self.config.network);
    ...
    pub async fn create_wallet_from_mnemonic(
        ...
        info!("Wallet created successfully - Node ID: {}, Address: {}", node_id, address);
    ...
    pub async fn get_balance(&self) -> Result<(u64, u64)> {
        ...
        info!("Balance retrieved - On-chain: {} sats, Lightning: {} sats", 
    ...
    pub async fn buy_airtime(
        ...
        info!("Buying airtime for {} sats to phone: {} via provider: {:?}", 
        ...
        info!("Airtime purchase initiated - Invoice: {}, Payment Hash: {}", 
              &invoice[..50], payment_hash);
...
impl Drop for LightningEngine {
    fn drop(&mut self) {
        ...
        if let Some(node) = self.node.try_write().ok().and_then(|mut guard| guard.take()) {
            let _ = node.stop();
        }
    }
}
...
mod tests {
    ...
    async fn test_wallet_creation() {
        ...
        let (node_id, address) = engine.create_wallet_from_mnemonic(mnemonic, "test-wallet").await.unwrap();
