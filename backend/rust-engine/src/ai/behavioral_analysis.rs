...
impl BehavioralAnalyzer {
    ...
    pub fn analyze_anomaly(
        ...
        let overall_score = (time_anomaly
            + location_anomaly
            + device_anomaly
            + transaction_anomaly
            + behavioral_anomaly)
    ...
    fn calculate_time_anomaly(&self, baseline: &UserBehavior, current: &UserBehavior) -> f64 {
        let time_diff =
            self.calculate_time_difference(&baseline.time_patterns, &current.time_patterns);
        let session_diff = self.calculate_session_difference(
            &baseline.time_patterns.session_timing,
            &current.time_patterns.session_timing,
        );
    ...
    fn calculate_location_anomaly(&self, baseline: &UserBehavior, current: &UserBehavior) -> f64 {
        let country_diff = if baseline.location_patterns.primary_country
            != current.location_patterns.primary_country
        {
            1.0
        } else {
            0.0
        ...
        let city_diff =
            if baseline.location_patterns.primary_city != current.location_patterns.primary_city {
                0.8
            } else {
                0.0
            };

        let consistency_diff = (baseline.location_patterns.location_consistency
            - current.location_patterns.location_consistency)
            .abs();
    ...
    fn calculate_transaction_anomaly(
        &self,
        baseline: &UserBehavior,
        current: &UserBehavior,
    ) -> f64 {
        let frequency_diff = (baseline.transaction_frequency - current.transaction_frequency).abs()
            / baseline.transaction_frequency.max(0.1);
        let amount_diff =
            (baseline.average_transaction_amount - current.average_transaction_amount).abs()
                / baseline.average_transaction_amount.max(1.0);

        (frequency_diff + amount_diff) / 2.0
    }

    fn calculate_behavioral_anomaly(&self, baseline: &UserBehavior, current: &UserBehavior) -> f64 {
        let session_diff = (baseline.session_duration as f64 - current.session_duration as f64)
            .abs()
            / baseline.session_duration.max(1) as f64;
    ...
    fn calculate_time_difference(&self, baseline: &TimePatterns, current: &TimePatterns) -> f64 {
        let hour_diff = self
            .calculate_u8_list_difference(&baseline.most_active_hours, &current.most_active_hours);
        let day_diff = self
            .calculate_u8_list_difference(&baseline.most_active_days, &current.most_active_days);

        (hour_diff + day_diff) / 2.0
    }

    fn calculate_session_difference(
        &self,
        baseline: &SessionTiming,
        current: &SessionTiming,
    ) -> f64 {
        let length_diff =
            (baseline.average_session_length as f64 - current.average_session_length as f64).abs()
                / baseline.average_session_length.max(1) as f64;
        let start_diff =
            (baseline.typical_session_start as f64 - current.typical_session_start as f64).abs()
                / 24.0;
        let end_diff =
            (baseline.typical_session_end as f64 - current.typical_session_end as f64).abs() / 24.0;
        let freq_diff = (baseline.session_frequency - current.session_frequency).abs()
            / baseline.session_frequency.max(0.1);
