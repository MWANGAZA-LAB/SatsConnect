...
use crate::multi_currency::fiat_providers::{FiatProvider, MpesaProvider, AirtelMoneyProvider, MTNProvider};
use anyhow::Result;
...
impl CurrencyService {
    ...
    pub fn new() -> Self {
        let mut fiat_providers: HashMap<Currency, Box<dyn FiatProvider + Send + Sync>> = HashMap::new();
    ...
    pub async fn get_exchange_rate(&self, currency: Currency) -> Result<ExchangeRate> {
        ...
        info!("Exchange rate for {}: {} sats per {}", currency.code(), rate.rate, currency.code());
        Ok(rate)
    }
    ...
    pub async fn fiat_to_sats(&self, fiat_amount: f64, currency: Currency) -> Result<u64> {
        ...
        if sats < currency.min_sats() {
            return Err(anyhow::anyhow!("Amount too small. Minimum: {} sats", currency.min_sats()));
        }
        if sats > currency.max_sats() {
            return Err(anyhow::anyhow!("Amount too large. Maximum: {} sats", currency.max_sats()));
        }
