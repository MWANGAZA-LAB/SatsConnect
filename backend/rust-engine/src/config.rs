...
impl LightningConfig {
    ...
    fn default_esplora_url() -> String {
        match env::var("BITCOIN_NETWORK").unwrap_or_default().to_lowercase().as_str() {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        match self.network {
            Network::Bitcoin => {
                if self.esplora_url.contains("testnet") || self.esplora_url.contains("regtest") {
                    return Err(anyhow::anyhow!("Mainnet configuration with testnet Esplora URL"));
                }
            }
            Network::Testnet => {
                if !self.esplora_url.contains("testnet") && !self.esplora_url.contains("regtest") {
                    return Err(anyhow::anyhow!("Testnet configuration with mainnet Esplora URL"));
                }
            }
            Network::Regtest => {
                if !self.esplora_url.contains("127.0.0.1") && !self.esplora_url.contains("localhost") {
                    return Err(anyhow::anyhow!("Regtest configuration with remote Esplora URL"));
                }
            }
            Network::Signet => {
                if !self.esplora_url.contains("signet") {
                    return Err(anyhow::anyhow!("Signet configuration with non-signet Esplora URL"));
                }
            }
        }
        ...
        if self.lightning_node.min_channel_size >= self.lightning_node.max_channel_size {
            return Err(anyhow::anyhow!("Minimum channel size must be less than maximum channel size"));
        }
        
        if self.lightning_node.channel_reserve >= self.lightning_node.min_channel_size {
            return Err(anyhow::anyhow!("Channel reserve must be less than minimum channel size"));
        }
